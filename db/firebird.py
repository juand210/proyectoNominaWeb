    ...
    try:
        ...
        if tipo_liq == "liquida_vacacion":
            ...
            sql = """
                ...
                L.FECHA_ULT_VACACION AS FECHA_INICIAL, L.FECHA_PER_VAC_INI DESDE,
                L.FECHA_PER_VAC_FIN  HASTA,'SIN' AS ESTADO_VACACION,L.COD_PERSONAL,
                L.COD_VINCULOLB,E.NOMBRE,TN.DESCRIPCION AS NOMBRETN,L.COD_TIPO_NOMINA
                FROM NR_VINCULO L
                WHERE (L.COD_PERSONAL) LIKE (:COD_PERSONAL) AND
                L.ESTADO NOT IN ('RETIRADO','LIQUIDADO')
                AND ( (L.COD_PERSONAL||L.COD_VINCULOLB)
                NOT IN (SELECT DISTINCT (COD_PERSONAL||COD_VINCULOLB) FROM NR_PERSONAL_VAC_DTL))
            """
            params = (cod_personal,)
